...
## Supabase 준비
1) Supabase 프로젝트 생성
2) SQL Editor에서 `supabase/schema.sql (v3: 정책 IF NOT EXISTS 제거)` 실행
...
## 상품가입(텔러)
- 텔러 로그인 후 **상품가입**에서 예적금/대출/투자/카드/보험 신청 저장
- 카드 번호 자동 생성: 국내(9496), VISA(4816), MASTER(5927)
- 대출 이자: 고정 4%
- 주택청약 만기: 9999년(데모)


## iPad 서식(QR)
- 텔러에서 QR 생성 → iPad로 스캔 → 서명/제출
- 토큰 유효: 10분, 제출 후 만료
